 ...
 * - 2025-11-11 04:36:09 [浦野一輝]：設定方法・使用方法・よくある質問シートを大幅に簡素化 - アプリURL自動反映を削除、メニューから不要な項目を削除、デプロイ手順を詳細化（ステップバイステップ）、通知設定の説明を削除、よくある質問を必要最小限に削減、バリデーション機能の説明を削除
 * 
 * 【push時の変更履歴（大きな変更のみ）】
...
function setupSettingSheet(sheet, driveFolderId) {
  ...
  sheet.getRange(2, 2).setNote('デプロイしたアプリのURLをここに手動で入力してください。\nデプロイ方法は「② Webアプリとしてデプロイ」セクションを参照してください。');
  ...
  const data = [
    ...
    ['   - スプレッドシートの上部メニューから「拡張機能」→「Apps Script」を選択'],
    ['   - または、以下のURLをブラウザで開く'],
    ['   - URL形式: https://script.google.com/home/projects/[スクリプトID]/edit'],
    ['   - スクリプトIDは、このスプレッドシートに紐づいているApps ScriptのIDです'],
    ['   - スクリプトIDの確認方法: Apps Scriptエディタを開いた状態で、URLの「/projects/」と「/edit」の間の文字列'],
    [''],
    ['【デプロイ手順（詳細版）】'],
    [''],
    ['ステップ1: デプロイメニューを開く'],
    ['1. Apps Scriptエディタの画面上部にある「デプロイ」ボタンをクリック'],
    ['   - 画面右上付近に「デプロイ」というボタンがあります'],
    [''],
    ['2. 「新しいデプロイ」を選択'],
    ['   - 「デプロイ」ボタンをクリックすると、ドロップダウンメニューが表示されます'],
    ['   - その中から「新しいデプロイ」をクリック'],
    [''],
    ['ステップ2: デプロイの種類を選択'],
    ['1. 「種類の選択」の右側にある歯車アイコン（⚙️）をクリック'],
    ['   - 画面中央に「種類の選択」という項目があります'],
    ['   - その右側に歯車アイコンがあります'],
    [''],
    ['2. 「ウェブアプリ」を選択'],
    ['   - 歯車アイコンをクリックすると、選択肢が表示されます'],
    ['   - その中から「ウェブアプリ」を選択'],
    [''],
    ['ステップ3: デプロイの設定'],
    ['1. 「説明」欄に適切な説明を入力'],
    ['   - 例: 「初期デプロイ」「勤怠管理アプリ v1.0」など'],
    ['   - この説明は、後でデプロイ履歴を確認する際に役立ちます'],
    [''],
    ['2. 「次のユーザーとして実行」を「自分」に設定'],
    ['   - 「次のユーザーとして実行」というドロップダウンがあります'],
    ['   - その中から「自分」を選択'],
    ['   - これにより、あなたのアカウントでスクリプトが実行されます'],
    [''],
    ['3. 「アクセスできるユーザー」を「全員」に設定'],
    ['   - 「アクセスできるユーザー」というドロップダウンがあります'],
    ['   - その中から「全員」を選択'],
    ['   - これにより、誰でもWebアプリにアクセスできるようになります'],
    ['   - ※注意: 「自分」を選択すると、あなた以外はアクセスできません'],
    [''],
    ['ステップ4: デプロイを実行'],
    ['1. 「デプロイ」ボタンをクリック'],
    ['   - 画面下部にある「デプロイ」ボタンをクリック'],
    ['   - 初回デプロイ時は、権限の承認が求められる場合があります'],
    [''],
    ['2. 権限の承認（初回のみ）'],
    ['   - 「承認が必要です」という画面が表示された場合'],
    ['   - 「権限を確認」ボタンをクリック'],
    ['   - Googleアカウントでログイン'],
    ...
    [''],
    ['3. WebアプリのURLをコピー'],
    ['   - デプロイが完了すると、「WebアプリのURL」が表示されます'],
    ['   - このURLをコピーして保存してください'],
    ['   - 例: https://script.google.com/macros/s/AKfycb.../exec'],
    [''],
    ['ステップ5: アプリURLを設定シートに記録'],
    ['1. このスプレッドシートに戻る'],
    [''],
    ['2. 「設定方法」シートのB2セルに、コピーしたURLを貼り付け'],
    ['   - B2セルをクリック'],
    ['   - コピーしたURLを貼り付け（Ctrl+V または Cmd+V）'],
    ['   - Enterキーを押して確定'],
    [''],
    ['③ 動作確認'],
    [''],
...
function setupUsageSheet(sheet) {
  const data = [
    ...
    [''],
    ['Q: Webアプリにアクセスできません'],
    ['A: 設定方法シートを一から実施してください。'],
    ['   - 「設定方法」シートの「② Webアプリとしてデプロイ」セクションを確認'],
    [''],
    ['以上で使用方法の説明は終わりです。'],
...
function setupFaqSheet(sheet) {
  const data = [
    ...
    [''],
    ['Q7: Webアプリにアクセスできません'],
    ['A7: 設定方法シートを一から実施してください。'],
    ['   1. 「設定方法」シートの「② Webアプリとしてデプロイ」セクションを確認'],
    ['   2. デプロイが正しく行われているか確認'],
    ['   3. WebアプリのURLが正しいか確認'],
    ...
    ['A8: 設定方法シートを一から実施してください。'],
    ['   1. 「設定方法」シートの「① 初期設定の実行」を再実行'],
    ['   2. 「設定方法」シートの「② Webアプリとしてデプロイ」を再実行'],
    ['   3. Apps Scriptの権限が正しく設定されているか確認'],
    [''],
    ['【データ管理について】'],
    [''],
    ['Q11: データを削除したいです'],
